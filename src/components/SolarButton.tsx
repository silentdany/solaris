...
export const SolarButton = (props: Props) => {
  const { url, ext, title, item } = props;

  return (
    <div>
      <Link href={url} passHref>
        <a title={title} target={ext ? '_blank' : '_self'} rel="noreferrer">
          <button className="lg:px-18 group-solar-btn btn relative h-12 w-52 min-w-max overflow-hidden rounded-xl border-0 bg-white px-4 shadow-xl transition-all hover:bg-white md:px-12 lg:h-16">
